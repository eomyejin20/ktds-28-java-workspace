...
public class ComicsBookCafe {
	// 멤버변수:
	private List<ComicsBook> comicsBooks;
	// 매출액
	private int take;
	...
	public ComicsBookCafe() {
		this.comicsBooks = new ArrayList<>();;
	}
	
	public void addBook(ComicsBook comicsBook) {
		this.comicsBooks.add(comicsBook);
	}
	...
	public void displayBooks() {
		...
		// 예외처리
		for (int i = 0; i < this.comicsBooks.size(); i++) {
			ComicsBook book = this.comicsBooks.get(i);
			if(book != null) {
				String state = "대여가능";
				if (book.getIsRent()) {
					state = "대여중";
