...
public class Friends {
	...
	public List<Friend> findFriendsBy(int days) {
		...
		LocalDate beforeThreeDays = LocalDate.now().plusDays(-days);
		System.out.println("3일 이전의 날짜 : " + beforeThreeDays);
		
		// 오늘 날짜에서 3일 이후의 날짜를 구한다.
		LocalDate afterThreeDays = LocalDate.now().plusDays(days);
		System.out.println("3일 이후의 날짜: " + afterThreeDays);
		...
		for (int i = 0; i < this.friendList.size(); i++) {
			birth = this.friendList.get(i).getBirthdate().withYear(now.getYear());
			
			if (birth.isAfter(beforeThreeDays) && birth.isBefore(afterThreeDays)) {
